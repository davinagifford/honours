### anomaly_analysis_north.R
###
### Exploring the anomalies and how they differ for the samples north of the separation zone. 
###
### Created: 2025-08-08
...
anomaly_north$trip_month <- as.Date(anomaly_north$trip_month)


# Create the plot with a trend line
ggplot(anomaly_north, aes(x = trip_month, y = anomaly)) +
  geom_line(color = "steelblue", linewidth = 1) +
  geom_point(color = "darkred", size = 2) +
  geom_smooth(method = "loess", se = TRUE, color = "black", linetype = "dashed") +
  labs(
    title = "EAC CCI Anomaly Over Time with Trend Line - North",
    x = "Trip Month",
    y = "EAC CCI Anomaly"
  ) +
  theme_minimal() +
...
ggplot(anomaly_north, aes(x = trip_month, y = anomaly_z)) +
  geom_line() +
  geom_point(aes(color = is_strong), size = 2) +
  scale_color_manual(values = c("FALSE" = "black", "TRUE" = "red")) +
  geom_hline(yintercept = 0, linetype = "dashed") +
  labs(
    title = paste("EAC Climate Anomalies (z-score) with Strong Events Highlighted (Threshold =", threshold, ") - North"),
...
strong_anomalies_n <- anomaly_north %>%
  filter(is_strong)
...
ggplot(strong_anomalies_n, aes(x = direction)) +
  ...
  labs(
    title = "Direction of Strong Anomalies - North",
    x = "Direction", y = "Count"
  ) +
  theme_minimal()

# Run ANOVA
anova_model_n <- aov(anomaly_strength ~ season, data = anomaly_north)
summary(anova_model_n)

# Full LOESS-smoothed anomaly strength plot
ggplot(anomaly_north, aes(x = trip_month, y = anomaly_strength)) +
  geom_point(color = "black", alpha = 0.6) +
  geom_smooth(method = "loess", span = 0.2, se = FALSE, color = "green", linewidth = 1.2) +
  labs(
    title = "LOESS Smoothing of Anomaly Strength Over Time (z-score) - North",
    x = "Month",
...
ggplot(anomaly_north, aes(x = trip_month, y = anomaly_strength, color = direction)) +
  ...
  labs(
    title = "LOESS Smoothing of Anomaly Strength by Direction (z-score) - North",
    x = "Year",
    y = "Anomaly Strength (|z-score|)"
  ) +
...
anomaly_north$season <- factor(anomaly_north$season, levels = c("Winter", "Spring", "Summer", "Autumn"))

# seasonal breakdown of anomalies
# Basic boxplot
ggplot(anomaly_north, aes(x = season, y = anomaly, fill = season)) +
  geom_boxplot(alpha = 0.6) +
  labs(
    title = "EAC CCI Anomalies by Season - North",
    x = "Season",
    y = "EAC CCI Anomaly"
  ) +
  theme_minimal() +
