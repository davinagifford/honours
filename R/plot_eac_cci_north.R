...
cci_data <- readRDS(file.path("var", "eac_cci_north.rds"))
...
p <-
    ...
    geom_segment(data = month_data_t_n,
    ...
    geom_line(data = clim_points,
              mapping = aes(linetype = "Climatology",
                            colour = "Climatology"),
              linewidth = 1.1) +
    geom_point(data = month_data_t_n, size = 2.5, shape = 21, fill = "black") +
    ...
    labs(x = "Time",
         ...
         colour = NULL,
         linetype = NULL,
         title = "EAC copepod composition index north of Separation Zone (monthly average)")
ggsave(file.path("output", "eac_cci_north.png"),
...
png(file.path("output", "eac_cci_rda_envfit_north.png"),
...
png(file.path("output", "eac_cci_climatology_gam_terms_north.png"),
    width = 680, height = 680, res = 96)
plot(lm_fit, pages = 1)
invisible(dev.off())
...
p <-
    ...
    scale_x_datetime(date_breaks = "1 month", minor_breaks = NULL,
                     date_labels = "%b")
ggsave(file.path("output", "eac_cci_climatology_north.png"),
...
ggsave(file.path("output", "eac_cci_climatology_check_north.png"),
       plot = p, width = 800 / 96, height = 600 / 96, dpi = 96,
       device = png)
...
ggsave(file.path("output", "eac_cci_rda_envfit_north_ggplot.png"),
